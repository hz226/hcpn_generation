...
def json_to_cpn_xml(
    ...
    unique_counter = counter_start  # Simple incremental ID counter, for generating unique IDs.

    def generate_id(prefix: str) -> str:
        """Simple incremental ID generator, to produce unique 'IDxxxx' strings."""
        nonlocal unique_counter
        unique_counter += 1
        if prefix.lower() == "place":
            return f"ID{str(unique_counter).rjust(10,"1")}" # NEW ID+10 digits
    ...
    def gather_all_variables(json_data: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        for trans in json_data.get("transitions", []):
            for v in trans.get("variables", []):
                if v not in var_map.setdefault("INT", []):
                    var_map.setdefault("INT", []).append(v)
        return var_map
    
    def gather_all_global_variables(json_data: Dict[str, Any]) -> Dict[str, List[str]]:
        """
        Collects global variables from the JSON schema. This example lumps them into "INT".
        For a real usage, adapt as needed to read arc expressions or user config.
        Returns { "INT": [var1, var2, ...] } or similar.
        """
        var_map = {}
        for trans in json_data.get("transitions", []):
            for v in trans.get("globalVariables", []):
                var_map.setdefault("BOOL", []).append(v)
        return var_map
    ...
    def create_global_var_elements(block_elem: ET.Element, var_map: Dict[str, List[str]]):
        for cs_name, vars_list in var_map.items():
            if not vars_list:
                continue
            for v in vars_list:
                var_id = generate_id("globref")
                var_elt = ET.SubElement(block_elem, "globref", {"id": var_id})

                # color set reference
                tid = ET.SubElement(var_elt, "id")
                tid.text = v
                
                ml = ET.SubElement(var_elt, "ml")
                ml.text = "false" #TODO default type: BOOL
                
                # Provide a layout line
                layout_elt = ET.SubElement(var_elt, "layout")
                layout_elt.text = f"globref {v}={ml.text};"
        
    
    def export_port_place(json_data):
        # New get the Input and Output ports of a page
        in_arcs = set()
        out_arcs = set()
        for transition in json_data["transitions"]:
            for arc in transition.get("inArcs", []):
                in_arcs.add(arc["place"])
            for arc in transition.get("outArcs", []):
                out_arcs.add(arc["place"])

        in_places = in_arcs - out_arcs
        out_places = out_arcs - in_arcs
        return [in_places, out_places]

    def build_ports(place_name, place, ports_config):
    	# New add ports
        if len(ports_config) < 2:
            return

        inputs, outputs = ports_config[0], ports_config[1]

        if place_name not in inputs and place_name not in outputs:
            return

        port_type = "In" if place_name in inputs else "Out"

        # A minimal "type" child referencing the color set
        port_elt = ET.SubElement(
            place,
            "port",
            {"id": generate_id("type"), "type": port_type}
        )
        ...
        ET.SubElement(
            port_elt,
            "posattr",
            {"x": f"{px - 24.0:.6f}", "y": f"{py - 17.0:.6f}"}
        )
        # Usually fill/line/text can be blank
        ET.SubElement(port_elt, "fillattr", {"colour": "White", "pattern": "Solid", "filled": "false"})
        ET.SubElement(port_elt, "lineattr", {"colour": "Black", "thick": "0", "type": "Solid"})
        ET.SubElement(port_elt, "textattr", {"colour": "Black", "bold": "false"})
    ...
    # color_sets = default_decls + color_sets
    color_sets = list(set(default_decls + color_sets))
    ...
    var_global_map = gather_all_global_variables(json_data)
    create_global_var_elements(block_decls, var_global_map)
    ...
    # <pageattr> name
    pageattr = ET.SubElement(page, "pageattr", {"name": cpn_name}) # New - change constant value to variable cpn_name

    # We'll track the IDs for places/transitions to link arcs
    place_name_to_id = {}
    transition_name_to_id = {}
	# New add port information
    ports_config = export_port_place(json_data)
    # -------------------------------------------------------------------
    # 4. PLACES
    # -------------------------------------------------------------------
    for place_info in json_data.get("places", []):
        ...
        #New
        build_ports(place_name, place_elt, ports_config)
    ...
    for trans_info in json_data.get("transitions", []):
        ...
        code_text.text = trans_info.get("code") or ""
    ...
    def get_transition_info (page, trans_id):
        all_transitions = page.findall("trans")
        x_location = "0.000000"
        y_location = "0.000000"
        for tran in all_transitions:
            if tran.get("id") == trans_id:
                current_trans = tran
                x_location = float(tran.find("posattr").get("x"))
                y_location = float(tran.find("posattr").get("y"))
                break
    ...
    for trans_info in json_data.get("transitions", []):
        ...
        for arc_info in trans_info.get("inArcs", []):
            ...
            # New
            x_annot, y_annot = get_transition_info(page, trans_id)
            ET.SubElement(annot_elt, "posattr", {
                "x": f"{x_annot - 120:.6f}",  # New updates
                "y": f"{y_annot - 10:.6f}"
            })
    ...
    return doctype_line + xml_str, unique_counter

def apply(json_path: str, cpn_name: str = "myNet", counter_start=100):
    """
    ...
    cpn_xml, counter = json_to_cpn_xml(data, coords, cpn_name, counter_start)

    return cpn_xml, counter
