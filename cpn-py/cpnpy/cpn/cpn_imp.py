...
class Transition:
    def __init__(self, name: str, guard: Optional[str] = None, variables: Optional[List[str]] = None,
                 transition_delay: int = 0, code: str = "", global_variables: Optional[List[str]] = None):
        self.name = name
    ...
    def __repr__(self):
        ...
        global_vars_str = ", ".join(self.global_variables) if self.global_variables else "None"
        return f"Transition(name='{self.name}', guard='{guard_str}', variables=[{vars_str}], global_variables=[{global_vars_str}], delay={self.transition_delay}, code='{self.code}')"
    ...
    def __copy__(self):
        ...
        result.global_variables = self.global_variables[:]
    ...
    def __deepcopy__(self, memo):
        ...
        result.global_variables = copy.deepcopy(self.global_variables, memo)
        result.transition_delay = self.transition_delay
        result.code = copy.deepcopy(self.code, memo)
        return result
