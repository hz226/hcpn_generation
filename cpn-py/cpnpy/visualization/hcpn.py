...
class HCPNGraphViz:
    ...
    def apply(
        ...
        self.graph.attr(rankdir=layout_direction)
        ...
        for (parent_mod, parent_trans), child_mod in hcpn.substitutions.items():
            ...
            # Update: only link to the first transitions in the child's module
    ...
    def save(self, filename: str):
        ...
        if os.path.abspath(final_path) != os.path.abspath(out_path):
            os.replace(out_path, final_path)   # atomic overwrite
        return final_path
